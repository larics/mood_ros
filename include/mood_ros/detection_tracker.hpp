...
public:
    ...
    const std::vector<centroid_t>& t_inputCentroids)
  {
    ...
    if (m_centroidMap.empty()) {
      ...
      ROS_WARN_THROTTLE(THROTTLE_DURATION,
                        "CentroidTracker - no objects currently stored");
...
private:
  ...
  void case_match_input_to_centroids(const std::vector<centroid_t>& t_inputCentroids)
  {
    ROS_INFO_THROTTLE(THROTTLE_DURATION,
                      "CantroidTracker::case_match_input_to_centroids");
  ...
  {
    ROS_WARN_THROTTLE(THROTTLE_DURATION,
                      "CantroidTracker::case_handle_empty_centroid_input");
    ...
    for (auto& map_entry : m_disappearedCentroidMap) {
      ...
      ROS_WARN_STREAM_THROTTLE(THROTTLE_DURATION,
                               "Object: " << m_centroidMap[centroid_index]
                                          << " missing for " << disappeared_frames
                                          << " frames");
